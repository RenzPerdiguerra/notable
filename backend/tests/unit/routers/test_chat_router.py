def test_create_session_endpoint(user, client):
...
def test_update_session_endpoint(user, client):
...
def test_delete_session_endpoint(user, client):
...
def test_create_message_endpoint(user, client):
...
def test_list_messages_endpoint(user, client):
    create_resp = client.post("/chat/sessions", json={"user_id": user.id, "title": "Chat"})
    session_id = create_resp.json()["id"]
